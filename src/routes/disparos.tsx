...
function DisparosPage() {
  ...
  return (
    <>
      <PageHeader
        title="Disparos"
        subtitle="Envie um fluxo para todos os leads de uma etiqueta"
        actions={
          <button onClick={() => setCreating(true)} className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground hover:bg-primary/90">
            <Plus className="h-3.5 w-3.5" /> Realizar Disparo
          </button>
        }
      />
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${
                      ...
                      b.status === "running" ? "bg-warning/10 text-warning" :
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg rounded-lg border border-border bg-card p-6 shadow-xl">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Fluxo</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Etiqueta (Tag)</label>
            ...
            {count !== null && (
              <p className="mt-1 text-[11px] text-muted-foreground">{count} lead(s) ativo(s) com essa etiqueta</p>
            )}
          </div>
          ...
          <p className="text-[11px] text-muted-foreground">
            ...
            O sistema sorteia um intervalo entre o mínimo e o máximo a cada disparo para evitar bloqueios do WhatsApp.
          </p>
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button onClick={submit} disabled={saving || !flowId || !tagId} className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
              {saving ? "Disparando..." : "Iniciar disparo"}
            </button>
          </div>
        </div>
      </div>
